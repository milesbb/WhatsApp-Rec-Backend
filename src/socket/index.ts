  ...
  newUser.on("connectReceiveInfo", (payload: UsernameWithId) => {
    ...
    newUser.on("checkChats", async (payload: string[]) => {
      try {
        ...
        const chats = await ChatsModel.find({ members: objectIdArray }); //INSIDE HERE NEEDS MONGOOSE QUERY TERMS TO FIND ALL CHATS THAT HAVE PARTICIPANTS EXACTLY EQUAL TO PAYLOAD PARTICIPANT ARRAY
